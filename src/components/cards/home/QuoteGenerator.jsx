...
const QuoteGenerator = () => {
  ...
  return (
    <div className="w-full flex h-full flex-col items-center justify-between text-primary">
      <div className="flex flex-col items-start justify-center gap-2 w-full min-h-[120px]">
        <h3 className="text-lg">Byte Sized Wisdom:</h3>
        <p className="text-xl text-secondary min-h-[60px] font-secondary">
          {loading ? (
      ...
      <button
        ...
        className={`border border-border rounded-lg w-full text-lg py-2 transition-all duration-300 ease-in-out cursor-pointer ${
          loading
            ...
            : "hover:bg-border active:translate-y-1"
        }`}
      >
        {loading ? (
