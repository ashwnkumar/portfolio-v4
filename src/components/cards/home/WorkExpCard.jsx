...
const WorkExpCard = () => {
  ...
  return (
    <section className="w-full h-full flex flex-col gap-4 text-start">
      <h3 className="text-xl md:text-2xl">Work Experience:</h3>
        ...
        {workExp.map(({ id, position, company, from, to, work, skills }) => {
          ...
          return (
            <div key={id} className="w-full pb-3">
              ...
              >
                <div>
                  <h4 className="text-lg md:text-xl text-primary font-secondary font-medium">
                    {position}
                  </h4>
                    ...
                    <>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {skills.map((skill, idx) => (
                          <span
                            key={idx}
                            className="text-sm rounded-full px-4 py-1 border border-border"
                          >
                            {skill}
                          </span>
                        ))}
